...
class Robot {
...

	{
		...
		if (egg > butter) {
			System.out.println(butter + "個のオムレツを作ります");
		} else {
			System.out.println(egg + "個のオムレツを作ります");
	...
	String makeEggDishes(int flourNum, int sugarNum, int eggNum, int butterNum) {
		...
		int egg2 = eggNum - 2;
		int butter2 = butterNum - 80;

		String menu = null;

		if (flour >= 0 && sugar >= 0 && egg2 >= -1 && butter2 >= 0) {
			menu = "クッキー";
		} else if (egg2 >= 0 && butter2 >= -75) {
			menu = "オムレツ";
		} else if (egg2 >= -1) {
			menu = "ゆで卵";
		} else {
...
public class RobotMaker {
	...
	public static void main(String[] args) throws IOException {
		...
		if (menu != null) {
			System.out.println(menu + "を作ります");
		} else {
			System.out.println("何も作りません。");
		}
