...
public class WarehouseManager {
	...
	public static void main(String[] args) throws IOException {
		...
		for (int j = 0; j < jankenArray2.length; j++) {
			...
			if ((stoneCount2 == 2) && (num == 2)) {
				j--;
				continue;
			} else if ((scissorsCount2 == 2) && (num == 2)) {
				j--;
				continue;
			} else if ((paperCount2 == 2) && (num == 2)) {
				j--;
				continue;
			}
			jankenArray2[j] = num;
		...
		for (int count = 0; count < 5; count++) {
			...
			do {
				...
				openNum1 = Integer.parseInt(str2) - 1;
			...
			//一度選んだコンテナの番号が添え字になっている要素を、0にする。
			//選んだことのあるコンテナが再び選ばれた時に、値が0となっているため、上記の処理で	jankenArray1[openNum1] == 0となった時、同じコンテナが選ばれたという処理にできる。
